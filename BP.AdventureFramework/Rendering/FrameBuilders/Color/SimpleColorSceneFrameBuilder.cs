...
namespace BP.AdventureFramework.Rendering.FrameBuilders.Color
{
    ...
    public sealed class SimpleColorSceneFrameBuilder : ISceneFrameBuilder
    {
        ...
        public IFrame Build(Room room, ViewPoint viewPoint, PlayableCharacter player, string message, CommandHelp[] contextualCommands, KeyType keyType, int width, int height)
        {
            ...
            if (player.Items.Any())
                gridStringBuilder.DrawWrapped("You have: " + StringUtilities.ConstructExaminablesAsSentence(player.Items.Cast<IExaminable>().ToArray()), leftMargin, lastY + linePadding, availableWidth, TextColor, out _, out lastY);
