...
namespace AdventureFramework.Rendering.Frames
{
    ...
    public class RegionMapFrame : Frame
    {
        ...
        public override string BuildFrame(int width, int height, FrameDrawer drawer)
        {
            ...
            // add title
            scene.Append(drawer.ConstructWrappedPaddedString(this.Region.Name, width, true));
            ...
            {
                // get map
                String map = this.MapDrawer.ConstructRegionMap(this.Region, width, height - (drawer.DetermineLinesInString(scene.ToString()) + 5));
