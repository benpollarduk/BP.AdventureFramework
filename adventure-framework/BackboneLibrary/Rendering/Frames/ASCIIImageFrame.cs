...
namespace AdventureFramework.Rendering.Frames
{
    ...
    public class ASCIIImageFrame : Frame
    {
        ...
        {
            get { return ASCIIImageFrame.grayscaleASCIIDictionary; }
            private set { ASCIIImageFrame.grayscaleASCIIDictionary = value; }
        }

        /// <summary>
        /// Get or set the grayscale ASCII dictionary
        /// </summary>
        private static Dictionary<Byte, Char> grayscaleASCIIDictionary = ASCIIImageFrame.CreateDefaultASCIIDictionary();
        ...
        public override string BuildFrame(int width, int height, FrameDrawer drawer)
        {
            ...
            // hold ascii drawing
            String asciiDrawing = String.Empty;

            // itterate each line
            for (Int32 index = 0; index < this.Lines.Length; index++)
            {
                // append line
                asciiDrawing += drawer.ConstructCentralisedString(this.Lines[index], width);
            }

            // hold buffer height
            Int32 bufferHeight = height - (asciiDrawing.Length / width) - 2;
            ...
            // hold buffer
            String buffer = drawer.ConstructPaddedArea(width, bufferHeight / 2);
        ...
        public static String[] ConvertToASCIIImageString(Int32 width, Int32 height, Bitmap image, Int16 contrast)
        {
            ...
            {
                // hold width
                resizedWidth = (Int32)(image.Width * ((Double)width / (Double)image.Width));

                // hold height
                resizedHeight = (Int32)(image.Height * ((Double)width / (Double)image.Width));
            }
            else
            {
                // hold width
                resizedWidth = (Int32)(image.Width * ((Double)height / (Double)image.Height));

                // hold height
                resizedHeight = (Int32)(image.Height * ((Double)height / (Double)image.Height));
            }

            // redefine width based on cell size of courier new
            resizedWidth = (Int32)((Double)resizedWidth * courierNewRatio);
            ...
            for (Int32 rowIndex = 0; rowIndex < height; rowIndex++)
            {
                ...
                {
                    // get ASCII
                    line += ASCIIImageFrame.ConvertPixelsToASCII(contrast, paddedBMP.GetPixel(columnIndex, rowIndex));
                }
        ...
        protected static String ConvertPixelToASCII(Color pixel, Int16 contrast)
        {
            // get average colour
            Int32 average = ((pixel.R + pixel.G + pixel.B) / 3) * (pixel.A / 255);
            ...
            return ASCIIImageFrame.GrayscaleASCIIDictionary[(Byte)average].ToString();
        }
        ...
        protected static String ConvertPixelsToASCII(Int16 contrast, params Color[] pixels)
        {
            ...
            // itterate all pixels
            for (Int32 pixelIndex = 0; pixelIndex < pixels.Length; pixelIndex++)
            {
                // add r
            ...
            return ASCIIImageFrame.ConvertPixelToASCII(Color.FromArgb(r /= pixels.Length, g /= pixels.Length, b /= pixels.Length), contrast);
        }
        ...
        public static ASCIIImageFrame Create(Image image, Int32 width, Int32 height)
        {
            // return created frame
            return ASCIIImageFrame.Create(image as Bitmap, width, height, 0);
        }
        ...
        /// <param name="contrast">The contrast between light and dark. Default is 0, use 1 - 100 to increase the contrast, use -1 - -100 to decrease the contrast</param>
        /// <returns>A new ASCIIImageFrame created from the parameters</returns>
        public static ASCIIImageFrame Create(Image image, Int32 width, Int32 height, Int16 contrast)
        {
            // return created frame
            return ASCIIImageFrame.Create(image as Bitmap, width, height, contrast);
        }
        ...
        /// <param name="height">Specify the height of the ASCII image</param>
        /// <returns>A new ASCIIImageFrame created from the parameters</returns>
        public static ASCIIImageFrame Create(Bitmap image, Int32 width, Int32 height)
        {
            // create frame
            ASCIIImageFrame frame = new ASCIIImageFrame();

            // set lines
            frame.Lines = ASCIIImageFrame.ConvertToASCIIImageString(width - 4, height - 3, image, 0);
        ...
        /// <param name="contrast">The contrast between light and dark. Default is 0, use 1 - 100 to increase the contrast, use -1 - -100 to decrease the contrast</param>
        /// <returns>A new ASCIIImageFrame created from the parameters</returns>
        public static ASCIIImageFrame Create(Bitmap image, Int32 width, Int32 height, Int16 contrast)
        {
            // create frame
            ASCIIImageFrame frame = new ASCIIImageFrame();

            // set lines
            frame.Lines = ASCIIImageFrame.ConvertToASCIIImageString(width - 4, height - 3, image, contrast);
