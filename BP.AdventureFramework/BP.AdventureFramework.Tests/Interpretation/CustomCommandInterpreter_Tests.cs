...
namespace BP.AdventureFramework.Tests.Interpretation
{
    ...
    public class CustomCommandInterpreter_Tests
    {
        ...
        {
            var interpreter = new CustomCommandInterpreter();
            overworld.Commands = new[] { new CustomCommand(new CommandHelp("Test", string.Empty), g => new Reaction(ReactionResult.Error, string.Empty)) };
            var game = Game.Create(string.Empty, string.Empty, string.Empty, x => overworld, () => new PlayableCharacter(Identifier.Empty, Description.Empty), null).Invoke();
        ...
        {
            var interpreter = new CustomCommandInterpreter();
            overworld.Commands = new[] { new CustomCommand(new CommandHelp("Test", string.Empty), g =>
            {
                Assert.IsTrue(true);
